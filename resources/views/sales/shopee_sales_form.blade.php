...
<div class="rounded-md shadow-lg bg-orange-600 p-2 lg:w-1/2 justify-center">
    ...
    <div class="bg-gray-200 px-2 py-2 flex flex-col">
        ...
        <form method="POST" action="{{route('addshopeesales')}}">
            @csrf
            <div>
                <input type="text" name="order_id" placeholder="Order ID" class="h-6 w-full mb-0.5 text-xs">
            </div>
            <div>
                <input type="text" name="full_name" placeholder="Full Name" class="h-6 w-full mb-0.5 text-xs">
            </div>
            <div>
                <input type="text" name="customers_address" placeholder="Full Address" class="h-6 w-full mb-0.5 text-xs">
            </div>
            <div>
                <input type="text" name="phone_number" placeholder="Phone Number" class="h-6 w-full mb-0.5 text-xs">
            </div>
            <div>
                <input type="number" name="charges_and_fees" placeholder="Charges & Fees" class="h-6 w-full mb-0.5 text-xs">
            </div>
            <div class="mb-2">
                <select class="h-6 text-xs py-0" name="status">
                    <option value="" disabled selected>Choose Status</option>
                    <option value=4 disabled selected>Delivered</option>
