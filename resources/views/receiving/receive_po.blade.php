...
<div class="ml-1 mt-2">
    <div class="flex items-center">
        <form method="GET" action="{{ route('receivepoform') }}">
            ...
            <input type="submit" class="bg-teal-400 text-gray-200 hover:bg-teal-600 p-1 ml-1 rounded-md shadow-md">
        </form>
    </div>
</div>
...
<div class="bg-gray-900 rounded-md px-4 py-4 max-w-screen-sm mt-4">
    <div class="bg-gray-200 px-4 py-4">
        ...
        <div class="flex flex-row mb-4">
            <div>
                <h2 class="text-gray-800 font-bold">P.O Number: <span class="text-blue-600 font-bold">{{ $purchaseOrder -> po_number }}</span></h2>
            </div>

            <div class="ml-auto">
                <h2 class="text-gray-800 font-bold">Status: <span class="text-red-700 font-bold capitalize">{{ $purchaseOrder->systemStatus->status }}</span></h2>
            </div>
            <div class="ml-auto">
                @if($purchaseOrder->del_status == 0)
                <h2 class="text-gray-800 font-bold">Del Status: <span class="text-red-700 font-bold capitalize">No</span></h2>
                @else
                <h2 class="text-gray-800 font-bold">Del Status: <span class="text-red-700 font-bold capitalize">Yes</span></h2>
                @endif
            </div>
        </div>
        <h2 class="text-gray-800 mb-4 font-bold">Supplier Name: <span class="text-blue-600 font-bold">{{ $purchaseOrder->supplier_name }}</span></h2>
        ...
        <div class="flex flex-row">
            ...
            <div class="ml-auto mt-4 font-bold mr-4">
                <th>Total Amount:</th>
                <td><span class="text-red-600">{{ $totalAmount }}</span></td>
            </div>
        </div>
