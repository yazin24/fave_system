...
<div class="mt-2">
    {{-- <form method="POST" action="{{route('deliveredshopeestatus', ['shopeeSale' => $shopeeSale -> id])}}">
        @csrf
        @method('PUT') --}}
    <div class="bg-blue-900 rounded-sm p-1 max-w-screen-sm mt-4">
        <div class="bg-gray-200 px-4 py-4"> 
            <h2 class="font-bold text-sm mb-1">Order ID: {{$lazadaOrder -> order_number}}</h2>
            ...
            <div class="bg-white-900 text-gray-900 mt-4"> 
                <table class="w-full shadow-md bg-gray-400">
                    ...
                    <tbody class="bg-gray-300">
                        @foreach($lazadaOrder -> lazadaOrderProducts as $index => $lazadaItem)
                        <tr class="h-10">   
           ...
           <div class="mt-4 w-full">
            ...
            @elseif($lazadaOrder -> status == 4)
            <h2 class="w-full text-gray-200 bg-blue-900 hover:bg-blue-800 font-bold text-sm p-1 rounded-sm shadow-md text-center">Completed</h2>
            @endif
           </div>
