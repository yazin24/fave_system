...
<h2 class="font-bold">Manual Purchase Order</h2>

<div class="mt-4 font-bold">
    {{-- <form method="POST" action="{{route('updatedelstatusmanual', ['manualPurchase' => $manualOrder -> id])}}">
        @csrf
        @method('PUT') --}}
        <div class="bg-gray-900 rounded-sm p-1 max-w-screen-sm mt-4 font-bold">
        <div class="bg-gray-200 px-4 py-4"> 

                <h2 class="text-xs text-gray-800 mb-4 font-bold">P.O Number: <span class="text-blue-600 font-bold">{{$manualOrder -> po_number}}</span></h2>
                <h2 class="text-xs text-gray-800 mb-4 font-bold">Full Name: <span class="text-blue-600 font-bold">{{$manualOrder -> customers_name}}</span></h2>
                <h2 class="text-xs text-gray-800 mb-4 font-bold">Contact Number: <span class="text-blue-600 font-bold">{{$manualOrder -> contact_number}}</span></h2>
                <h2 class="text-xs text-gray-800 mb-4 font-bold">Address: <span class="text-blue-600 font-bold">{{$manualOrder -> address}}</span></h2>
                <h2 class="text-xs text-gray-800 mb-4 font-bold">Purchase Type: <span class="text-blue-600 font-bold">{{$manualOrder -> purchase_type}}</span></h2>
                @if($manualOrder -> status == 7 && $manualOrder -> isApproved == 1)
               ...
               <div class="bg-white-900 text-gray-900 mt-1"> 
                    <table class="w-full shadow-md bg-gray-400">
                        ...
                        <tbody class="bg-gray-300">
                            @foreach($manualOrder -> manualPurchaseOrderProducts as $index => $item)
                            <tr class="h-10">   
                ...
                <div class="flex">
                    ...
                    <div class="ml-auto mt-4 font-bold mr-4">
                        ...
                        <td><span class="text-xs text-red-600">₱{{$totalOrderAmount}}</span></td>
                    </div>
                 </div>
         </div>
        {{-- </form> --}}
         @if($manualOrder -> isApproved == 3)
         <div class="flex gap-1 mt-1">
             <div>
             <form method="POST" action="{{route('approvemanual', ['manualPurchase' => $manualOrder -> id])}}">
                 @csrf
         ...
         <div>
             <form method="POST" action="{{route('disapprovemanual', ['manualPurchase' => $manualOrder -> id])}}">
                 @csrf
         ...
         @elseif($manualOrder -> isApproved == 2)
         ...
         @elseif($manualOrder -> isApproved == 1)
         <div class="flex flex-col w-full">
             <button class="w-full bg-teal-500 hover:bg-teal-600 font-bold p-1 rounded-sm shadow-md text-gray-200 text-xs"><a href="{{route('manualreceipt', ['manualPurchase' => $manualOrder])}}"><i class="fa-solid fa-print mr-0.5"></i>Generate Receipt</a></button>
         </div>
         @endif
    </div>
</div>
